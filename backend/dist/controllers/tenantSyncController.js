...
exports.getTenantSyncConfig = exports.bootstrapTenant = void 0;
...
const bootstrapTenant = async (req, res) => {
    try {
        ...
        for (const role of roles) {
            const createdRole = await db_1.default.role.upsert({
        ...
        for (const perm of permissions) {
            const createdPerm = await db_1.default.permission.upsert({
        ...
        let localTenant;
        if (tenant.id) {
            localTenant = await db_1.default.tenant.upsert({
                where: { id: tenant.id },
        ...
        if (adminUser.id) {
            existingUser = await db_1.default.user.findUnique({ where: { id: adminUser.id } }).catch(() => null);
        }
        if (!existingUser && adminEmail) {
            existingUser = await db_1.default.user.findUnique({ where: { email: adminEmail } }).catch(() => null);
        }
        if (!existingUser) {
        ...
        if (existingUser) {
            ...
            createdAdminUser = await db_1.default.user.update({
                where: { id: existingUser.id },
                data: userUpdatePayload
            });
        }
        else {
            createdAdminUser = await db_1.default.user.create({
                data: {
        ...
        for (const page of defaultPages) {
            await db_1.default.customPage.upsert({
                where: {
                    tenantId_slug: {
                        tenantId: localTenant.id,
                        slug: page.slug
        ...
        for (const moduleName of defaultModules) {
            await db_1.default.adminPermission.upsert({
                where: {
                    tenantId_module: {
                        tenantId: localTenant.id,
                        module: moduleName
