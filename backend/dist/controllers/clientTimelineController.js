...
exports.getClientTimeline = void 0;
const db_1 = require("../config/db");
function humanizePaymentMode(mode) {
    if (!mode)
        return 'Online Payment';
    const m = mode.toUpperCase().trim();
    if (m === 'CUSTOM_PRO_RATA')
        return 'Custom Plan (Adjusted Validity/Amount)';
    if (m === 'ADMIN_ASSIGNED' || m === 'MANUAL_ADMIN')
        return 'Admin Manual Assignment';
    if (m === 'ONLINE_RAZORPAY' || m === 'RAZORPAY')
        return 'Online Payment (Razorpay)';
    if (m === 'OFFLINE_CASH' || m === 'CASH')
        return 'Cash';
    if (m === 'OFFLINE_BANK_TRANSFER' || m === 'BANK_TRANSFER' || m === 'NEFT' || m === 'RTGS')
        return 'Bank Transfer (NEFT/RTGS)';
    if (m === 'OFFLINE_UPI' || m === 'UPI')
        return 'UPI Transfer';
    if (m === 'OFFLINE_CHEQUE' || m === 'CHEQUE')
        return 'Cheque';
    return mode.replace(/_/g, ' ');
}
function humanizeEsignMode(mode) {
    if (!mode)
...
const getClientTimeline = async (req, res) => {
    try {
        ...
        const client = await db_1.dynamicDb.Client.findById(clientId).lean();
        if (!client) {
            return res.status(404).json({ success: false, message: 'Client not found.' });
        }
        const user = client.userId ? await db_1.dynamicDb.User.findById(client.userId).lean() : null;
        // 2. Fetch Real-time Activity Logs
        const rawLogs = await db_1.dynamicDb.ActivityLog
            .find({ targetClientId: client._id || client.id })
            .sort({ timestamp: -1 })
            .limit(parseInt(limit, 10) || 100)
            .lean();
        const realTimeEvents = (rawLogs || []).map((log) => ({
        ...
        if (client.userId) {
            try {
                const userAudits = await db_1.dynamicDb.AuditLog.find({
                    $or: [
                ...
                for (const audit of userAudits) {
                    ...
                    const aTime = new Date(audit.timestamp || audit.createdAt || client.createdAt || signupDate);
                    historicalEvents.push({
        ...
        // 3D. Subscriptions
        const subscriptions = await db_1.dynamicDb.Subscription.find({
            $or: [{ clientId: client._id }, { clientId: client.id }, { clientId: client.userId }]
        })
            .populate('planId')
            .lean();
        // 3E. Payments
        const payments = await db_1.dynamicDb.Payment.find({
            $or: [{ clientId: client._id }, { clientId: client.id }, { clientId: client.userId }]
        })
            .populate('planId')
            .lean();
        ...
        for (const pay of payments) {
            ...
            const isStaffAssigned = Boolean(pay.assignedByAdminName ||
            ...
            const matchingSub = subscriptions.find((s) => !handledSubIds.has(String(s._id)) &&
                (String(s.planId?._id || s.planId) === String(pay.planId?._id || pay.planId))) || (subscriptions.length === 1 && !handledSubIds.has(String(subscriptions[0]._id)) ? subscriptions[0] : undefined);
            let validityStr = '';
            if (matchingSub) {
            ...
            if (isStaffAssigned) {
                ...
                if (validityStr)
                    parts.push(`Validity: ${validityStr}`);
                if (cleanNote)
        ...
        for (const sub of subscriptions) {
            ...
            const subTime = new Date(sub.createdAt || client.createdAt);
        ...
        // 3F. Support Tickets
        try {
            const tickets = await db_1.dynamicDb.SupportTicket.find({
                $or: [{ clientId: client._id }, { clientId: client.id }, { clientId: client.userId }]
            }).lean();
        ...
        if (search && search.trim()) {
            ...
            allEvents = allEvents.filter((e) => e.title.toLowerCase().includes(q) ||
        ...
        const hasPayment = payments.some((p) => p.status === 'SUCCESS' || p.status === 'VERIFIED');
        const hasActiveSub = subscriptions.some((s) => s.status === 'ACTIVE' && new Date(s.endDate) > currentTime);
        const milestones = [
            {
                step: 1,
                key: 'REGISTRATION',
            ...
            },
            {
                ...
                timestamp: payments[0]?.paymentDate || subscriptions[0]?.createdAt || client.createdAt || null,
                detail: hasPayment
            ...
            },
            {
                ...
                timestamp: hasActiveSub ? (subscriptions[0]?.createdAt || subscriptions[0]?.startDate || null) : null,
        ...
        const loginEventsCount = allEvents.filter((e) => e.category === 'AUTH' && (e.action === 'LOGIN' || e.action === 'LOGIN_OTP' || e.action === 'LOGIN_SUCCESS')).length;
