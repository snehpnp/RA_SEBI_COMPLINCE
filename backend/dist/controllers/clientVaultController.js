...
const vaultExcelService_1 = require("../services/vaultExcelService");
/**
 ...
 */
function buildClientFolderName(client) {
 ...
 */
function formatTradeTargets(s) {
    const tgts = [];
    if (s.target1 !== undefined && s.target1 !== null && s.target1 !== '')
        tgts.push(`₹${s.target1}`);
    if (s.target2 !== undefined && s.target2 !== null && s.target2 !== '')
        tgts.push(`₹${s.target2}`);
    if (s.target3 !== undefined && s.target3 !== null && s.target3 !== '')
        tgts.push(`₹${s.target3}`);
 ...
 */
function matchReportToClientStocks(report, clientStockSymbols, clientStockNames) {
 ...
 */
function generateResearchReportPdf(report, tenant) {
    return new Promise((resolve, reject) => {
        try {
            const doc = new pdfkit_1.default({ compress: false, margins: { top: 40, bottom: 40, left: 45, right: 45 } });
            const buffers = [];
...
// ============================================================================
const listClientVaults = async (req, res) => {
    try {
        const tenantId = req.user?.tenantId;
        if (!tenantId)
            return res.status(400).json({ success: false, message: 'Tenant context is missing.' });
        const { search, planStatusFilter = 'ALL', page = '1', limit = '100' } = req.query;
        ...
        const [totalClients, clients] = await Promise.all([
            db_1.default.Client.countDocuments(query),
            db_1.default.Client.find(query).sort({ createdAt: -1 }).skip(skip).limit(limitNum).lean()
        ]);
        ...
        const [subCounts, docCounts, recCounts, agrCounts, activePlansList] = await Promise.all([
            db_1.default.Subscription.aggregate([
            ...
            db_1.default.ClientDocument.aggregate([
                { $match: { clientId: { $in: clientIds } } },
                { $group: { _id: '$clientId', count: { $sum: 1 } } }
            ]),
            db_1.default.ClientCallRecording.aggregate([
                { $match: { clientId: { $in: clientIds } } },
                { $group: { _id: '$clientId', count: { $sum: 1 } } }
            ]),
            db_1.default.Agreement.aggregate([
                { $match: { clientId: { $in: clientIds } } },
                { $group: { _id: '$clientId', count: { $sum: 1 } } }
            ]),
            db_1.default.Subscription.find({
                clientId: { $in: clientIds },
                status: 'ACTIVE',
                endDate: { $gt: now }
            }).populate('plan', 'name').lean()
        ]);
        const subMap = new Map(subCounts.map((s) => [String(s._id), s]));
        const docMap = new Map(docCounts.map((d) => [String(d._id), d.count]));
        const recMap = new Map(recCounts.map((r) => [String(r._id), r.count]));
        const agrMap = new Map(agrCounts.map((a) => [String(a._id), a.count]));
        // Map active plan names per client
        const clientActivePlansMap = new Map();
        for (const sub of (activePlansList || [])) {
            const cId = String(sub.clientId);
            const planName = sub.plan?.name || 'Active Plan';
            const existing = clientActivePlansMap.get(cId) || [];
            if (!existing.includes(planName))
...
exports.listClientVaults = listClientVaults;
// ============================================================================
// 2. GET CLIENT VAULT DETAILS (8 Sub-Folders + Virtual Files & Purchase Dates)
// ============================================================================
const getClientVaultDetails = async (req, res) => {
    try {
        const tenantId = req.user?.tenantId;
        ...
        const client = await db_1.default.Client.findOne({ _id: clientId, tenantId }).lean();
        if (!client) {
        ...
        const [clientProfile, userAccount, subscriptions, payments, documents, agreements, recordings, activityLogs, researchReports] = await Promise.all([
            ...
            client.userId ? db_1.default.User.findById(client.userId).lean() : null,
            db_1.default.Subscription.find({ clientId: client._id }).populate('plan').sort({ createdAt: -1 }).lean(),
            db_1.default.Payment.find({ clientId: client._id }).sort({ createdAt: -1 }).lean(),
            db_1.default.ClientDocument.find({ clientId: client._id }).sort({ uploadedAt: -1 }).lean(),
            db_1.default.Agreement.find({ clientId: client._id }).sort({ signedAt: -1 }).lean(),
            db_1.default.ClientCallRecording.find({ clientId: client._id, tenantId }).sort({ callDate: -1, createdAt: -1 }).lean(),
            db_1.default.ActivityLog.find({ targetClientId: client._id }).sort({ timestamp: -1 }).limit(100).lean(),
            db_1.default.ResearchReport.find({ tenantId, status: 'PUBLISHED' }).sort({ publishedAt: -1 }).limit(50).lean()
        ]);
        // Trade signals related strictly to client's subscribed plans
        const planIds = (subscriptions || [])
            .map((s) => s.planId || s.plan?._id)
            .filter(Boolean);
        let signals = [];
        if (planIds.length > 0) {
            signals = await db_1.default.Signal.find({ planId: { $in: planIds } })
                .populate('stockId')
                .sort({ createdAt: -1 })
                .limit(200)
                .lean();
        ...
        const clientStockSymbols = [];
        const clientStockNames = [];
        signals.forEach((s) => {
        ...
        const openTrades = signals.filter((s) => s.status === 'ACTIVE');
        const closedTrades = signals.filter((s) => s.status === 'CLOSED');
        const targetHitTrades = signals.filter((s) => s.closeStatus === 'TARGET_HIT');
        const slHitTrades = signals.filter((s) => s.closeStatus === 'SL_HIT');
        ...
        const enrichedSubscriptions = (subscriptions || []).map((sub) => {
            ...
            const matchingPayment = (payments || []).find((p) => {
                if (sub.paymentId && String(p._id) === String(sub.paymentId))
                    return true;
                if (sub.paymentId && p.transactionRef === sub.paymentId)
                    return true;
                if (String(p.planId) === String(sub.planId || sub.plan?._id))
                    return true;
                return false;
            });
            ...
            return {
                ...
                computedStatus,
                paymentMode: matchingPayment?.paymentMode || sub.paymentMode || 'Online',
                transactionRef: matchingPayment?.transactionRef || matchingPayment?.gatewayPaymentId || sub.paymentId || 'N/A',
                paymentStatus: matchingPayment?.status || 'SUCCESS',
                paymentId,
...
exports.getClientVaultDetails = getClientVaultDetails;
// ============================================================================
...
const uploadCallRecording = async (req, res) => {
    try {
        ...
        if (!tenantId)
            return res.status(400).json({ success: false, message: 'Tenant context missing.' });
        if (!clientId)
            return res.status(400).json({ success: false, message: 'Client ID missing.' });
        const client = await db_1.default.Client.findOne({ _id: clientId, tenantId }).lean();
        if (!client) {
        ...
        const { title = req.file.originalname, callType = 'ADVISORY', callDate = new Date(), durationSeconds = 0, callerStaffName = req.user?.name || req.user?.email || 'Compliance Staff', callerStaffId = req.user?.id, summary = '', isComplianceVerified = true } = req.body;
        ...
        const safeCallType = validCallTypes.includes(String(callType).toUpperCase())
            ...
            : 'ADVISORY';
        const newRecording = await db_1.default.ClientCallRecording.create({
            ...
            callerStaffId: callerStaffId && mongoose_1.default.Types.ObjectId.isValid(callerStaffId) ? new mongoose_1.default.Types.ObjectId(callerStaffId) : undefined,
            callerStaffName,
...
const deleteCallRecording = async (req, res) => {
    try {
        ...
        if (!tenantId)
            return res.status(400).json({ success: false, message: 'Tenant context missing.' });
        const recording = await db_1.default.ClientCallRecording.findOne({
            _id: recordingId,
            clientId,
            tenantId
        });
        ...
        if (recording.fileUrl) {
            const physicalPath = (0, pdfService_1.resolveAttachmentFilePath)(recording.fileUrl);
            if (physicalPath && fs_1.default.existsSync(physicalPath)) {
                try {
        ...
        await db_1.default.ClientCallRecording.findByIdAndDelete(recordingId);
        // Activity Log
        await (0, activityService_1.logActivity)({
            tenantId,
            actorId: req.user?.id,
...
exports.downloadSingleInvoice = downloadSingleInvoice;
// ============================================================================
...
const downloadAgreementPdf = async (req, res) => {
    try {
        ...
        const agreement = await db_1.default.Agreement.findOne({ clientId }).sort({ signedAt: -1 }).lean();
        if (agreement?.agreementUrl) {
            const physical = (0, pdfService_1.resolveAttachmentFilePath)(agreement.agreementUrl);
            if (physical && fs_1.default.existsSync(physical)) {
                res.setHeader('Content-Type', 'application/pdf');
...
const exportClientVaultZip = async (req, res) => {
    try {
        ...
        const [clientProfile, userAccount, subscriptions, payments, documents, agreements, recordings, activityLogs, researchReports] = await Promise.all([
            ...
            db_1.default.Agreement.find({ clientId: client._id }).sort({ signedAt: -1 }).lean(),
            db_1.default.ClientCallRecording.find({ clientId: client._id, tenantId }).sort({ callDate: -1, createdAt: -1 }).lean(),
            db_1.default.ActivityLog.find({ targetClientId: client._id }).sort({ timestamp: -1 }).limit(500).lean(),
            db_1.default.ResearchReport.find({ tenantId, status: 'PUBLISHED' }).sort({ publishedAt: -1 }).limit(50).lean()
        ]);
        // Trade signals related strictly to client's subscribed plans
        const planIds = (subscriptions || []).map((s) => s.planId || s.plan?._id).filter(Boolean);
        const signals = planIds.length > 0
            ? await db_1.default.Signal.find({ planId: { $in: planIds } }).populate('stockId').sort({ createdAt: -1 }).lean()
            : [];
        // Extract all unique stock symbols & names from client's signals (open or closed)
        const clientStockSymbols = [];
        const clientStockNames = [];
...
`.trim();
        ...
        // ------------------------------------------------------------------------
        try {
            const masterExcelBuffer = await (0, vaultExcelService_1.generateMultiTabClientExcel)({
                client,
                clientProfile,
                subscriptions,
                payments,
        ...
        for (const p of (payments || []))
            targetInvoiceIds.add(String(p._id));
        for (const s of (subscriptions || [])) {
            if (s.paymentId)
                targetInvoiceIds.add(String(s.paymentId));
            else
                targetInvoiceIds.add(String(s._id));
        }
        for (const id of Array.from(targetInvoiceIds)) {
            try {
                const invoicePdf = await (0, invoiceGenerator_1.generateInvoicePdf)(id);
        ...
        for (const report of clientResearchReports) {
            try {
                if (report.isAttachment && report.fileUrl) {
                    const physical = (0, pdfService_1.resolveAttachmentFilePath)(report.fileUrl);
                    if (physical && fs_1.default.existsSync(physical)) {
                        const ext = path_1.default.extname(report.fileUrl) || '.pdf';
        ...
        for (const doc of documents || []) {
            if (doc.fileUrl) {
                const physical = (0, pdfService_1.resolveAttachmentFilePath)(doc.fileUrl);
                if (physical && fs_1.default.existsSync(physical)) {
                    const ext = path_1.default.extname(doc.fileUrl) || path_1.default.extname(doc.fileName) || '.pdf';
        ...
        for (const agr of agreements || []) {
            if (agr.agreementUrl) {
                const physical = (0, pdfService_1.resolveAttachmentFilePath)(agr.agreementUrl);
                if (physical && fs_1.default.existsSync(physical)) {
        ...
        if (!appendedAgreementPdf && (client.agreementSigned || (agreements && agreements.length > 0))) {
            try {
                const agrPdf = await (0, pdfService_1.generateAgreementPdf)(String(client._id), { isSigned: true });
        ...
        for (const rec of recordings || []) {
            if (rec.fileUrl) {
                const physical = (0, pdfService_1.resolveAttachmentFilePath)(rec.fileUrl);
                if (physical && fs_1.default.existsSync(physical)) {
                    const ext = path_1.default.extname(rec.fileUrl) || path_1.default.extname(rec.fileName) || '.mp3';
...
exports.exportClientVaultZip = exportClientVaultZip;
// ============================================================================
...
const exportSingleFolder = async (req, res) => {
    try {
        ...
        if (folderKey === '01_Basic_Profile' || folderKey === 'basic-profile' || folderKey === '01' || folderKey === 'profile') {
            const clientProfile = await db_1.default.ClientProfile.findOne({ clientId: client._id }).lean();
            const profileRows = [
                ...
                { key: 'DigiLocker / KRA Verified', value: client?.kraVerified ? 'YES' : 'NO' }
            ];
            const excelBuffer = await (0, vaultExcelService_1.generateSingleSheetExcel)('Client_Profile', [
                { header: 'Profile Field / Metric', key: 'key', width: 28 },
                { header: 'Verified Client Detail', key: 'value', width: 45 }
            ], profileRows, 'FF1E3A8A' // Blue
            );
        ...
        if (folderKey === '02_Subscriptions_&_Invoices' || folderKey === 'subscriptions' || folderKey === '02' || folderKey === 'invoices') {
            ...
            const excelBuffer = await (0, vaultExcelService_1.generateSingleSheetExcel)('Subscriptions_&_Invoices', [
        ...
        if (folderKey === '03_Trade_Signals' || folderKey === 'trade-signals' || folderKey === '03' || folderKey === 'signals') {
            const subscriptions = await db_1.default.Subscription.find({ clientId: client._id }).lean();
            const planIds = subscriptions.map((s) => s.planId).filter(Boolean);
            const signals = planIds.length > 0
                ? await db_1.default.Signal.find({ planId: { $in: planIds } }).populate('stockId').sort({ createdAt: -1 }).lean()
                : [];
            const rows = signals.map((s) => {
                ...
                const tgts = [s.target1, s.target2, s.target3].filter((t) => t !== null && t !== undefined && !isNaN(Number(t)));
                const targetsStr = tgts.length > 0 ? tgts.map((t) => `₹${t}`).join(' / ') : `₹${s.target1 || '-'}`;
                return {
            ...
            const excelBuffer = await (0, vaultExcelService_1.generateSingleSheetExcel)('Trade_Signals', [
        ...
        if (folderKey === '04_Research_Reports' || folderKey === 'research-reports' || folderKey === '04' || folderKey === 'reports') {
            ...
            const signals = planIds.length > 0
                ? await db_1.default.Signal.find({ planId: { $in: planIds } }).populate('stockId').sort({ createdAt: -1 }).lean()
                : [];
            const [researchReports, tenant] = await Promise.all([
                db_1.default.ResearchReport.find({ tenantId, status: 'PUBLISHED' }).sort({ publishedAt: -1 }).limit(100).lean(),
                db_1.default.Tenant.findById(tenantId).lean()
            ]);
            const clientStockSymbols = [];
            const clientStockNames = [];
            signals.forEach((s) => {
            ...
            const archive = (typeof archiver === 'function')
                ...
                : new archiver.ZipArchive({ zlib: { level: 9 } });
            archive.pipe(res);
            const reportRows = clientResearchReports.map((r) => ({
                title: r.title || 'N/A',
            ...
            const excelBuffer = await (0, vaultExcelService_1.generateSingleSheetExcel)('Research_Reports_Index', [
        ...
        if (folderKey === '05_KYC_Documents' || folderKey === 'kyc-documents' || folderKey === '05' || folderKey === 'kyc') {
            const documents = await db_1.default.ClientDocument.find({ clientId: client._id }).sort({ uploadedAt: -1 }).lean();
            res.setHeader('Content-Type', 'application/zip');
            ...
            const excelBuffer = await (0, vaultExcelService_1.generateSingleSheetExcel)('KYC_Verification_Ledger', [
        ...
        if (folderKey === '06_Signed_Agreements' || folderKey === 'agreements' || folderKey === '06' || folderKey === 'agreement') {
            const agreements = await db_1.default.Agreement.find({ clientId: client._id }).sort({ signedAt: -1 }).lean();
            // Check if physical file exists for the primary agreement
            let pdfBuffer = null;
            if (agreements.length > 0 && agreements[0].agreementUrl) {
                const physical = (0, pdfService_1.resolveAttachmentFilePath)(agreements[0].agreementUrl);
                if (physical && fs_1.default.existsSync(physical)) {
                    pdfBuffer = fs_1.default.readFileSync(physical);
                }
            }
            // Fallback: Dynamically generate official signed agreement PDF
            if (!pdfBuffer) {
                try {
                    pdfBuffer = await (0, pdfService_1.generateAgreementPdf)(String(client._id), { isSigned: true });
                }
                catch (err) {
        ...
        if (folderKey === '07_Call_Recordings' || folderKey === 'recordings' || folderKey === '07' || folderKey === 'calls') {
            const recordings = await db_1.default.ClientCallRecording.find({ clientId: client._id, tenantId }).sort({ callDate: -1 }).lean();
            res.setHeader('Content-Type', 'application/zip');
            ...
            const excelBuffer = await (0, vaultExcelService_1.generateSingleSheetExcel)('Call_Recordings_Index', [
            ...
            for (const rec of recordings) {
                if (rec.fileUrl) {
                    const physical = (0, pdfService_1.resolveAttachmentFilePath)(rec.fileUrl);
                    if (physical && fs_1.default.existsSync(physical)) {
                        const ext = path_1.default.extname(rec.fileUrl) || '.mp3';
        ...
        if (folderKey === '08_Audit_Trail' || folderKey === 'audit-trail' || folderKey === '08' || folderKey === 'audit') {
            const activityLogs = await db_1.default.ActivityLog.find({ targetClientId: client._id }).sort({ timestamp: -1 }).limit(1000).lean();
            ...
            const excelBuffer = await (0, vaultExcelService_1.generateSingleSheetExcel)('Audit_Trail', [
...
exports.exportSingleFolder = exportSingleFolder;
// ============================================================================
...
const downloadSingleResearchReport = async (req, res) => {
    try {
        ...
        if (!clientId || !reportId)
            return res.status(400).json({ success: false, message: 'Client ID and Report ID required.' });
        const tenant = await db_1.default.Tenant.findById(tenantId).lean();
        // Check if reportId is a Signal ID with reportUrl
        const signal = await db_1.default.Signal.findOne({ _id: reportId, tenantId }).populate('stockId').lean();
        if (signal?.reportUrl) {
            const physical = (0, pdfService_1.resolveAttachmentFilePath)(signal.reportUrl);
            if (physical && fs_1.default.existsSync(physical)) {
        ...
        const report = await db_1.default.ResearchReport.findOne({ _id: reportId, tenantId }).lean();
        if (report) {
...
exports.downloadSingleResearchReport = downloadSingleResearchReport;
