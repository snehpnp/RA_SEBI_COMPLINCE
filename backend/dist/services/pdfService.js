...
exports.generateAgreementPdf = exports.getTenantComplianceAttachments = exports.generateInternalPolicyPdf = exports.generatePrivacyPolicyPdf = exports.generateTermsAndConditionsPdf = exports.resolveAttachmentFilePath = void 0;
 ...
 */
const resolveAttachmentFilePath = (filePathOrUrl) => {
    ...
    const fileName = path_1.default.basename(trimmed);
    const candidatePaths = [
        ...
        path_1.default.resolve(process.cwd(), '..', cleanPath),
        path_1.default.resolve(__dirname, '../../..', cleanPath),
        path_1.default.resolve(__dirname, '../../', cleanPath),
        path_1.default.resolve('a:/RA_SEBI_COMPLINCE/uploads', cleanPath.replace(/^uploads[/\\]?/, '')),
        path_1.default.resolve('a:/RA_SEBI_COMPLINCE/uploads/branding', fileName),
        path_1.default.join(__dirname, '../../../uploads/branding', fileName),
        path_1.default.join(process.cwd(), '../uploads/branding', fileName),
        path_1.default.join(process.cwd(), 'uploads/branding', fileName)
    ];
...
exports.resolveAttachmentFilePath = resolveAttachmentFilePath;
/**
 * Generates an official, SEBI-compliant Terms & Conditions PDF for the tenant advisory.
 */
const generateTermsAndConditionsPdf = async (tenant) => {
    return new Promise((resolve, reject) => {
        try {
            ...
            // Footer
            doc.fillColor('#94A3B8').fontSize(8).font('Helvetica-Oblique').text(`This is a regulatory document issued by ${companyName} in compliance with SEBI guidelines.`, 45, 740, { align: 'center', width: 522 });
            doc.end();
        }
        catch (err) {
            reject(err);
...
exports.generateTermsAndConditionsPdf = generateTermsAndConditionsPdf;
/**
 * Generates an official, SEBI-compliant Privacy Policy PDF for the tenant advisory.
 */
const generatePrivacyPolicyPdf = async (tenant) => {
    return new Promise((resolve, reject) => {
        try {
            const doc = new pdfkit_1.default({ compress: false, margins: { top: 40, bottom: 40, left: 45, right: 45 } });
            const buffers = [];
            ...
            // Footer
            doc.fillColor('#94A3B8').fontSize(8).font('Helvetica-Oblique').text(`This is a regulatory privacy policy issued by ${companyName} in compliance with SEBI guidelines.`, 45, 740, { align: 'center', width: 522 });
            doc.end();
        }
        catch (err) {
            reject(err);
...
const generateInternalPolicyPdf = async (tenant) => {
    return new Promise((resolve, reject) => {
        try {
            ...
            // Footer
            doc.fillColor('#94A3B8').fontSize(8).font('Helvetica-Oblique').text(`Confidential - Internal Supervisory Document for ${companyName} in compliance with SEBI Regulations.`, 45, 740, { align: 'center', width: 522 });
            doc.end();
        }
        catch (err) {
            reject(err);
...
exports.generateInternalPolicyPdf = generateInternalPolicyPdf;
/**
 ...
 */
const getTenantComplianceAttachments = async (tenantOrId) => {
    try {
        let tenant = tenantOrId;
        if (typeof tenantOrId === 'string') {
            tenant = await db_1.Tenant.findById(tenantOrId).lean();
        }
        if (!tenant)
            return [];
        const attachments = [];
        // 1. Terms & Conditions PDF
        const termsDiskPath = (0, exports.resolveAttachmentFilePath)(tenant.termsPdfUrl);
        if (termsDiskPath) {
            attachments.push({
            ...
            try {
                const termsBuffer = await (0, exports.generateTermsAndConditionsPdf)(tenant);
                attachments.push({
                    filename: 'Terms_and_Conditions.pdf',
                    content: termsBuffer,
                    contentType: 'application/pdf'
        ...
        const privacyDiskPath = (0, exports.resolveAttachmentFilePath)(tenant.privacyPdfUrl);
        if (privacyDiskPath) {
            attachments.push({
            ...
            try {
                const privacyBuffer = await (0, exports.generatePrivacyPolicyPdf)(tenant);
                attachments.push({
                    filename: 'Privacy_Policy.pdf',
...
exports.getTenantComplianceAttachments = getTenantComplianceAttachments;
const generateAgreementPdf = async (clientId, options) => {
    return new Promise(async (resolve, reject) => {
        try {
            let client = null;
            try {
                client = await db_1.Client.findById(clientId)
                    .populate('user')
                    .populate('userId')
                    .populate('profile')
                    .lean();
            ...
            if (!client) {
                try {
                    client = await db_1.Client.findOne({ userId: clientId })
                        .populate('user')
                        .populate('userId')
                        .populate('profile')
                        .lean();
            ...
            const cleanClientAddress = (profileObj.addressLine1
                ...
                : (client.address || '55, Chandigarh')).trim();
            let agreementText = tenant?.agreementContent ||
