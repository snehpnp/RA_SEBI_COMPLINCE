...
exports.generateSingleSheetExcel = exports.generateMultiTabClientExcel = void 0;
 ...
 * Tab 5: Audit_Trail
 */
const generateMultiTabClientExcel = async (data) => {
    ...
    const profileRows = [
        ...
        { key: 'DigiLocker / KRA Verified', value: data.client?.kraVerified ? 'YES' : 'NO' }
    ];
    ...
    rowH3.height = 24;
    const tradeRows = (data.tradeSignals || []).map((s) => {
        let pnl = '0.00%';
    ...
    const reportRows = (data.researchReports || []).map((r) => ({
        title: r.title || 'N/A',
    ...
    rowHKyc.height = 24;
    const kycRows = (data.documents || []).map((d) => ({
    ...
    rowHAgr.height = 24;
    const agrRows = (data.agreements || []).map((a) => ({
    ...
    rowH4.height = 24;
    const recRows = (data.recordings || []).map((r) => {
    ...
    rowH5.height = 24;
    const auditRows = (data.activityLogs || []).map((l) => ({
...
exports.generateMultiTabClientExcel = generateMultiTabClientExcel;
/**
 * Generates a Single-Sheet Excel Workbook (.xlsx)
 * Used when admin exports a specific subfolder directly.
 */
const generateSingleSheetExcel = async (sheetName, columns, rows, themeColorHex = 'FF1E293B') => {
    const wb = new exceljs_1.default.Workbook();
    wb.creator = 'SEBI Compliance System';
...
exports.generateSingleSheetExcel = generateSingleSheetExcel;
