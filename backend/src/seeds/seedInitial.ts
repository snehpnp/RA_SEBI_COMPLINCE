...
export async function seedInitial() {
  ...
  for (const role of roles) {
    const doc = await centralModels.Role.findOneAndUpdate(
      { name: role.name },
      { $set: role },
  ...
  for (const roleName of fullAdminRoles) {
    ...
    for (const permCode of Object.keys(permMap)) {
      ...
      await centralModels.RolePermission.findOneAndUpdate(
        ...
        { $setOnInsert: { roleId: rId, permissionId: pId } },
        { upsert: true, returnDocument: 'after' }
      );
    }
  }
  ...
  for (const permCode of poPerms) {
    ...
    await centralModels.RolePermission.findOneAndUpdate(
      ...
      { $setOnInsert: { roleId: poId, permissionId: pId } },
      { upsert: true, returnDocument: 'after' }
    );
  }
  ...
  for (const permCode of coPerms) {
    ...
    await centralModels.RolePermission.findOneAndUpdate(
      ...
      { $setOnInsert: { roleId: coId, permissionId: pId } },
      { upsert: true, returnDocument: 'after' }
    );
  }
  ...
  for (const permCode of researcherPerms) {
    ...
    await centralModels.RolePermission.findOneAndUpdate(
      ...
      { $setOnInsert: { roleId: researcherId, permissionId: pId } },
      { upsert: true, returnDocument: 'after' }
    );
  }
  ...
  for (const permCode of associatePerms) {
    const pId = permMap[permCode];
    await centralModels.RolePermission.findOneAndUpdate(
      { roleId: associateId, permissionId: pId },
      { $setOnInsert: { roleId: associateId, permissionId: pId } },
      { upsert: true, returnDocument: 'after' }
    );
  }
  ...
  for (const permCode of clientPerms) {
    const pId = permMap[permCode];
    await centralModels.RolePermission.findOneAndUpdate(
      { roleId: clientId, permissionId: pId },
      { $setOnInsert: { roleId: clientId, permissionId: pId } },
      { upsert: true, returnDocument: 'after' }
    );
  }
  ...
  const complianceUser = await centralModels.User.findOneAndUpdate(
  ...
  const researcherUser = await centralModels.User.findOneAndUpdate(
  ...
  for (const plan of plans) {
    await centralModels.Plan.findOneAndUpdate(
      { tenantId: plan.tenantId, name: plan.name },
      { $setOnInsert: plan },
      { upsert: true, returnDocument: 'after' }
    );
  }
