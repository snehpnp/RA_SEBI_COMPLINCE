...
export async function provisionAllTenantCollections(
  ...
  async function safeTenantUpsert(
  ...
  if (tenantData.id) {
    targetTenant = await safeTenantUpsert(
      targetPrisma,
      { id: tenantData.id },
      tenantPayload,
      tenantData.id
  ...
  try {
    ...
    if (rulesToSync && rulesToSync.length > 0) {
      for (const rule of rulesToSync) {
        ...
        const res = await db.collection('ComplianceRequirement').findOneAndUpdate(
          { serialNo },
          {
            $set: {
              serialNo,
    ...
    try {
      ...
      for (const reqRule of activeRules) {
        const period = getCompliancePeriod(reqRule.frequencyType || 'CONTINUOUS', now, targetTenant?.createdAt || now);
    ...
    try {
      ...
      for (const cat of categoriesToSync) {
        const catRes = await db.collection('PlanCategory').findOneAndUpdate(
          { tenantId, name: cat.name },
          {
            $set: {
              tenantId,
              name: cat.name,
        ...
        if (catRes && catRes._id) {
          ...
          if (cat.id) categoryMap[cat.id] = catRes._id.toString();
        }
      }
