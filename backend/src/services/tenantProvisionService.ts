...
export async function provisionAllTenantCollections(
  ...
  for (const role of roles) {
    const createdRole: any = await targetModels.Role.findOneAndUpdate(
      { name: role.name },
      { $set: role },
  ...
  for (const perm of permissions) {
    const createdPerm: any = await targetModels.Permission.findOneAndUpdate(
      { code: perm.code },
      { $set: perm },
      { upsert: true, returnDocument: 'after' }
    );
  ...
  for (const roleName of fullAdminRoles) {
    ...
    for (const permCode of Object.keys(permMap)) {
      ...
      await targetModels.RolePermission.findOneAndUpdate(
        { roleId: rId, permissionId: pId },
        { $set: { roleId: rId, permissionId: pId } },
        { upsert: true, returnDocument: 'after' }
      );
    }
  }
  ...
  if (poId) {
    ...
    for (const permCode of poPerms) {
      ...
      if (pId) {
        await targetModels.RolePermission.findOneAndUpdate(
          { roleId: poId, permissionId: pId },
          { $set: { roleId: poId, permissionId: pId } },
          { upsert: true, returnDocument: 'after' }
        );
      }
    }
  ...
  if (coId) {
    ...
    for (const permCode of coPerms) {
      ...
      if (pId) {
        await targetModels.RolePermission.findOneAndUpdate(
          { roleId: coId, permissionId: pId },
          { $set: { roleId: coId, permissionId: pId } },
          { upsert: true, returnDocument: 'after' }
        );
      }
    }
  ...
  if (researcherId) {
    ...
    for (const permCode of researcherPerms) {
      ...
      if (pId) {
        await targetModels.RolePermission.findOneAndUpdate(
          { roleId: researcherId, permissionId: pId },
          { $set: { roleId: researcherId, permissionId: pId } },
          { upsert: true, returnDocument: 'after' }
        );
      }
    }
  ...
  const tenantFilter: any = tenantData.id
    ? { _id: tenantData.id }
    : { email: tenantData.email };

  const targetTenant: any = await targetModels.Tenant.findOneAndUpdate(
    tenantFilter,
    { $set: tenantPayload },
    { upsert: true, returnDocument: 'after' }
  );
  ...
  if (targetUser) {
    createdAdminUser = await targetModels.User.findByIdAndUpdate(
      targetUser._id,
      { $set: userPayload },
  ...
  for (const p of permsToSync) {
    await targetModels.AdminPermission.findOneAndUpdate(
  ...
  for (const page of pagesToSync) {
    ...
    await targetModels.CustomPage.findOneAndUpdate(
  ...
  for (const t of templatesToSync) {
    if (!t.type) continue;
    await targetModels.EmailTemplate.findOneAndUpdate(
  ...
  try {
    ...
    if (rulesToSync && rulesToSync.length > 0) {
      for (const rule of rulesToSync) {
        await targetModels.ComplianceRequirement.findOneAndUpdate(
    ...
    const activeRequirements: any[] = await targetModels.ComplianceRequirement.find({ isActive: true }).lean();
    const now = new Date();
    for (const req of activeRequirements) {
      const period = getCompliancePeriod(req.frequencyType, now, targetTenant.createdAt || now);
  ...
  await ensureStates(targetModels.State).catch(() => {});

  // 11. Plan Category & Plans
  try {
    const categoryCount = await targetModels.PlanCategory.countDocuments({ tenantId });
    if (categoryCount === 0) {
      const defaultCategory: any = await targetModels.PlanCategory.create({
        tenantId,
  ...
  if (tenantData.systemSettings && tenantData.systemSettings.length > 0) {
    for (const setting of tenantData.systemSettings) {
      ...
      await targetModels.SystemSetting.findOneAndUpdate(
        { key: setting.key },
        {
          $set: {
            key: setting.key,
