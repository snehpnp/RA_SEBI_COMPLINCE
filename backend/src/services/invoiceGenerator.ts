 ...
 */
const setupFonts = (doc: PDFKit.PDFDocument) => {
  const possibleFontPaths = [
    {
      reg: path.join(__dirname, '../assets/fonts/Regular.ttf'),
      bold: path.join(__dirname, '../assets/fonts/Bold.ttf'),
      ital: path.join(__dirname, '../assets/fonts/Italic.ttf')
    },
    {
      reg: path.join(__dirname, '../../src/assets/fonts/Regular.ttf'),
      bold: path.join(__dirname, '../../src/assets/fonts/Bold.ttf'),
      ital: path.join(__dirname, '../../src/assets/fonts/Italic.ttf')
    },
    {
      reg: path.join(process.cwd(), 'src/assets/fonts/Regular.ttf'),
      bold: path.join(process.cwd(), 'src/assets/fonts/Bold.ttf'),
      ital: path.join(process.cwd(), 'src/assets/fonts/Italic.ttf')
    },
    {
      reg: path.join(process.cwd(), 'assets/fonts/Regular.ttf'),
      bold: path.join(process.cwd(), 'assets/fonts/Bold.ttf'),
      ital: path.join(process.cwd(), 'assets/fonts/Italic.ttf')
    },
    {
  ...
  for (const set of possibleFontPaths) {
    if (fs.existsSync(set.reg) && fs.existsSync(set.bold)) {
      try {
        doc.registerFont('Body', set.reg);
        doc.registerFont('Bold', set.bold);
        if (fs.existsSync(set.ital)) {
...
export const generateInvoicePdf = async (paymentId: string): Promise<Buffer> => {
  return new Promise(async (resolve, reject) => {
    try {
      ...
      if (!payment) {
        try {
          payment = await Payment.findOne({ $or: [{ _id: paymentId }, { transactionRef: paymentId }] }).populate('coupon').lean();
        } catch {}
      }
      ...
      const money = (n: number) => `${F.rupee}${n.toFixed(2)} /-`;
      ...
      const logoPath = resolveLogoPath(tenant.logoUrl) ||
                       resolveLogoPath('/logo-light.png') ||
                       resolveLogoPath('logo-light-full.png') ||
                       resolveLogoPath('logo-light.png');
      ...
      doc.fontSize(9).font(F.bold).fillColor('#000000').text(
        tenant.gst ? `GSTIN: ${tenant.gst}` : (tenant.sebiRegistration ? `SEBI REG NO: ${tenant.sebiRegistration}` : 'TAX INVOICE'),
        30,
        105,
      ...
      if (payment.transactionRef) doc.text(`Txn Ref: ${payment.transactionRef}`, 35, 439, { width: 305 });
