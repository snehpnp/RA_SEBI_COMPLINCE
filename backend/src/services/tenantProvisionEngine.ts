...
import { centralPrisma, tenantConnectionManager } from './tenantConnectionManager';
...
export class TenantProvisionEngine {
  private readonly ALL_PRISMA_COLLECTIONS = [
    'Tenant',
    'User',
    'Role',
    'Permission',
  ...
  public async provisionTenantFull(
    ...
    let createdTenantPrisma: PrismaClient | null = null;
    ...
    try {
      ...
      createdTenantPrisma = tenantConnectionManager.getPrismaByUri(mongoDbUrl);
      ...
      await centralPrisma.allCompany.deleteMany({ where: { tenantId } }).catch(() => {});
