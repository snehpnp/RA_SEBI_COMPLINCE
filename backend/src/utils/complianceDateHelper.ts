export const getAlertThresholdsForFrequency = (frequency: string) => {
 ...
 */
export const getRawCompliancePeriod = (frequencyType: string, refDate?: Date): CompliancePeriod => {
 ...
 */
export const calculateRawNextDueDate = (frequencyType: string, serialNo?: number, refDate?: Date): Date | null => {
  const now = refDate ? new Date(refDate) : new Date();
  const year = now.getFullYear();
...
export const getCompliancePeriod = (frequencyType: string, refDate?: Date, tenantCreatedAt?: Date): CompliancePeriod => {
  let period = getRawCompliancePeriod(frequencyType, refDate);
  if (tenantCreatedAt) {
    let safetyCounter = 0;
    while (period.dueDate < tenantCreatedAt && safetyCounter < 60) {
      const nextRef = new Date(period.dueDate.getTime() + 24 * 3600 * 1000);
      period = getRawCompliancePeriod(frequencyType, nextRef);
      safetyCounter++;
    }
  }
  return period;
...
export const calculateNextDueDate = (frequencyType: string, serialNo?: number, refDate?: Date, tenantCreatedAt?: Date): Date | null => {
  let nextDate = calculateRawNextDueDate(frequencyType, serialNo, refDate);
  if (tenantCreatedAt && nextDate) {
    ...
    while (nextDate && nextDate < tenantCreatedAt && safetyCounter < 60) {
      ...
      nextDate = calculateRawNextDueDate(frequencyType, serialNo, nextRef);
      safetyCounter++;
    }
  }
  return nextDate;
