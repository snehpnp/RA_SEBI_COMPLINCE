    ...
    <div className="flex flex-col sm:flex-row items-center justify-between px-4 py-3 border-t border-slate-300 dark:border-white/5 bg-white dark:bg-slate-900/30">
      ...
      <div className="flex items-center space-x-1">
        <button
          ...
          className="p-1 rounded-md text-slate-600 dark:text-slate-400 hover:text-slate-900 dark:hover:text-white hover:bg-slate-100 dark:hover:bg-white/10 dark:bg-slate-800 disabled:opacity-50 disabled:cursor-not-allowed transition"
        >
          ...
          {Array.from({ length: Math.min(5, totalPages) }, (_, i) => {
            ...
            return (
              <button
                ...
                className={`px-3 py-1 rounded-md text-xs font-medium transition ${ currentPage === pageNum ? 'bg-primary-600 text-white ' : 'text-slate-600 dark:text-slate-400 hover:text-slate-900 dark:hover:text-white hover:bg-slate-100 dark:hover:bg-white/10 dark:bg-slate-800' }`}
              >
                {pageNum}
              </button>
            );
