...
export default function TelegramSettingsTab() {
  ...
  return (
    <div className="space-y-8 animate-fade-in pb-16">
      ...
      <div className="bg-gradient-to-r from-blue-600 via-[#0088cc] to-sky-500 rounded-3xl p-6 sm:p-8 text-white shadow-xl shadow-blue-500/10 relative overflow-hidden">
        <div className="absolute top-0 right-0 w-96 h-96 bg-white/10 rounded-full blur-3xl -translate-y-1/2 translate-x-1/3 pointer-events-none" />
        
        <div className="relative z-10 flex flex-col md:flex-row md:items-center justify-between gap-6">
          <div className="space-y-2">
            <div className="inline-flex items-center space-x-2 bg-white/20 backdrop-blur-md px-3 py-1 rounded-full text-xs font-semibold tracking-wide">
              <Sparkles className="w-3.5 h-3.5 text-amber-300" />
              <span>Hybrid Telegram Auto-Hub (Zero-Ban & Plan-Wise Setup)</span>
            </div>
            <h2 className="text-2xl sm:text-3xl font-black tracking-tight">
              Telegram Signal Automation Center
            </h2>
            <p className="text-sm text-blue-100 max-w-2xl leading-relaxed">
              Connect via Phone & OTP to auto-discover or auto-create channels directly on your account, map dedicated groups per Subscription Plan, and dispatch market signals with 100% SEBI-compliant Bot delivery.
            </p>
          </div>
          ...
          <div className="flex flex-wrap items-center gap-3">
            <button
              ...
              disabled={loading}
              className="flex items-center space-x-2 bg-white/15 hover:bg-white/25 active:scale-95 text-white px-4 py-2.5 rounded-2xl text-xs font-bold backdrop-blur-md border border-white/20 transition shadow-sm"
            >
        ...
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-3 mt-6 pt-6 border-t border-white/15">
          ...
          <div className="bg-white/10 backdrop-blur-md rounded-2xl p-3.5 border border-white/10 flex items-center space-x-3">
            <div className={`p-2.5 rounded-xl ${authStatus.isConnected ? 'bg-emerald-500/20 text-emerald-300' : 'bg-white/20 text-blue-100'}`}>
              <Smartphone className="w-5 h-5" />
            </div>
            <div>
              <p className="text-[10px] uppercase font-bold tracking-wider text-blue-200">Admin Account</p>
              <p className="text-xs font-extrabold flex items-center gap-1.5 mt-0.5">
                {authStatus.isConnected ? (
                  <>
                    <span className="w-2 h-2 rounded-full bg-emerald-400 animate-pulse" />
                    <span>{authStatus.user?.firstName || authStatus.phone}</span>
                  </>
                ) : (
                  <span className="text-blue-100/70">Phone Not Linked</span>
                )}
              </p>
            </div>
          </div>
          ...
          {/* Bot Engine Status */}
          <div className="bg-white/10 backdrop-blur-md rounded-2xl p-3.5 border border-white/10 flex items-center space-x-3">
            <div className={`p-2.5 rounded-xl ${isConfigured ? 'bg-emerald-500/20 text-emerald-300' : 'bg-amber-500/20 text-amber-300'}`}>
              <ShieldCheck className="w-5 h-5" />
            </div>
            <div>
              <p className="text-[10px] uppercase font-bold tracking-wider text-blue-200">Safe Broadcast Bot</p>
              <p className="text-xs font-extrabold flex items-center gap-1.5 mt-0.5">
                {isConfigured ? (
                  <>
                    <span className="w-2 h-2 rounded-full bg-emerald-400" />
                    <span>Active (@Complince_signal_bot)</span>
                  </>
                ) : (
                  <span className="text-amber-200">Bot Token Needed</span>
                )}
              </p>
            </div>
          </div>
          ...
          <div className="bg-white/10 backdrop-blur-md rounded-2xl p-3.5 border border-white/10 flex items-center space-x-3">
            <div className="p-2.5 rounded-xl bg-white/20 text-blue-100">
              <Layers className="w-5 h-5" />
            </div>
            <div>
              <p className="text-[10px] uppercase font-bold tracking-wider text-blue-200">Active Groups & Plans</p>
              <p className="text-xs font-extrabold mt-0.5">
                {groups.length} Channel{groups.length !== 1 ? 's' : ''} • {planMatrix.length} Plan{planMatrix.length !== 1 ? 's' : ''}
              </p>
            </div>
          </div>
        </div>
