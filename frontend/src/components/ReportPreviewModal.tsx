          ...
          <div className="w-[210mm] min-h-[297mm] bg-white text-black shadow-lg flex flex-col relative scale-[0.6] sm:scale-[0.8] xl:scale-90 origin-top" ref={reportRef} style={{ padding: '25mm' }}>
            ...
            <div className="flex justify-between items-start mb-4">
              <div className="w-32 h-12 bg-black flex items-center justify-center rounded">
                 {user?.tenant?.logoUrl ? (
                    <img src={user.tenant.logoUrl.startsWith('http') ? user.tenant.logoUrl : `${(process.env.NODE_ENV === 'production' ? 'https://compliance.pnpuniverse.in/backend' : 'http://localhost:5000')}${user.tenant.logoUrl}`} alt="Logo" className="max-h-10 max-w-[100px] object-contain" />
                 ) : (
               ...
               <div className="mt-4 flex justify-end">
                  <div className="text-center w-40">
                     {user?.tenant?.coSignatureUrl ? (
                        <img crossOrigin="anonymous" src={user.tenant.coSignatureUrl.startsWith('http') ? user.tenant.coSignatureUrl : `${(process.env.NODE_ENV === 'production' ? 'https://compliance.pnpuniverse.in/backend' : 'http://localhost:5000')}${user.tenant.coSignatureUrl}`} alt="Signature" className="h-12 object-contain mx-auto mb-1" />
                     ) : (
