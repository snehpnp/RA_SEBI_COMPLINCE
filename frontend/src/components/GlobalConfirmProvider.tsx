...
export const GlobalConfirmProvider = ({ children }: { children: ReactNode }) => {
  ...
  return (
    <ConfirmContext.Provider value={{ confirm }}>
      ...
      {isOpen && (
        <div className="fixed inset-0 z-[100] flex items-center justify-center bg-slate-100 dark:bg-slate-950/80 backdrop-blur-md p-4 animate-in fade-in duration-200">
          <div className="bg-white dark:bg-slate-900 border border-slate-400 dark:border-white/10 rounded-2xl p-6 w-full max-w-sm shadow-2xl relative text-center animate-in zoom-in-95 duration-200">
            ...
            <div className="flex justify-center space-x-3">
              <button
                ...
                className="px-5 py-2.5 bg-slate-100 dark:bg-slate-800 hover:bg-slate-200 dark:bg-slate-700 text-sm font-bold rounded-xl transition-colors text-slate-700 dark:text-slate-300 w-full"
              >
                {options.cancelText || 'Cancel'}
              </button>
              <button
                onClick={handleConfirm}
                className="px-5 py-2.5 bg-primary-600 hover:bg-primary-500 text-sm font-bold rounded-xl transition-colors text-slate-900 dark:text-white shadow-lg shadow-primary-500/20 w-full"
              >
